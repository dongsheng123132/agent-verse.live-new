...
export const DetailModal: React.FC<DetailModalProps> = ({ 
  ...
  const isMulti = sortedCells.length > 1;
  const isLocked = sortedCells.some(c => c.status === 'LOCKED');
  const isBuyable = sortedCells.every(c => (!c.owner) || c.isForSale);
  ...
  const paymentAddress = "0x4eCf92bAb524039Fc4027994b9D88C2DB2Ee05E6";
  const coordString = sortedCells.map(c => `[${c.x},${c.y}]`).join(' + ');
...
`.trim();
  ...
  // Fix: Add origin to prevent playback errors (Error 153)
  const getSafeVideoUrl = (videoId: string) => {
      const origin = typeof window !== 'undefined' ? window.location.origin : '';
      return `https://www.youtube.com/embed/${videoId}?autoplay=1&mute=1&controls=1&rel=0&playsinline=1&enablejsapi=1&origin=${origin}`;
  };
                        ...
                        {(formData.name || isLocked) && (
                            <div className="prose prose-invert prose-sm max-w-none">
                                {formData.youtubeVideoId ? (
                                    <div className="w-full aspect-video bg-black rounded overflow-hidden border border-[#333] mb-4 relative">
                                        <iframe 
                                            width="100%" 
                                            height="100%" 
